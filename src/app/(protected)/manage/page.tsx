...
function ManageContent() {
  ...
  callIdcApiRef.current = async (action: string, params: Record<string, unknown> = {}, retry = true): Promise<Record<string, any>> => {
    ...
    const response = await fetch('/api/idc', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action, token, cookie, ...params }),
    });
    ...
    if (retry && (
      ...
      if (relogined) {
        ...
        const retryResp = await fetch('/api/idc', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action, token: freshAuth?.token || token, cookie: freshCookie, ...params }),
        });
        return retryResp.json();
      } else {
        setActionMsg({ type: 'error', text: 'IDCSmart 后台登录已过期，请返回登录页重新登录' });
