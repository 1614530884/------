...
import { AlertTriangle, Cpu, CheckCheck, Server as ServerIcon, History, AlertOctagon } from 'lucide-react';
import { Popover, PopoverContent, PopoverTrigger } from '@/components/ui/popover';
import { useCpuLimitAlerts } from '@/hooks/use-cpu-limit-alerts';
import { CpuLimitAlertHistoryDialog } from '@/components/cpu-limit/CpuLimitAlertHistoryDialog';
import type { CpuLimitAlertLog } from '@/lib/services/cpu-limit-manager';

interface CpuLimitAlertNotificationProps {
  /** 仅管理员启用（在 navbar 中控制） */
  enabled: boolean;
}
...
function AlertItem({ alert }: { alert: CpuLimitAlertLog }) {
  // windowMin === 0 表示解除失败告警（特殊场景）
  const isReleaseFailure = alert.windowMin === 0;
  const isNode = alert.level === 'node';
  const Icon = isReleaseFailure ? AlertOctagon : isNode ? ServerIcon : Cpu;
  const iconClass = isReleaseFailure
    ? 'bg-destructive/10 text-destructive'
    : isNode
      ? 'bg-destructive/10 text-destructive'
      : 'bg-amber-500/10 text-amber-600 dark:text-amber-400';
  const levelLabel = isReleaseFailure ? '解除失败' : isNode ? '节点级告警' : '实例级告警';
  const target = isNode
    ? `节点 ${alert.nodeName}`
    ...
    <div className="w-full px-3 py-2.5 flex items-start gap-2.5 border-b border-border/50 last:border-b-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1.5 flex-wrap">
          ...
          {isReleaseFailure ? (
            <span>
              解除失败重试 <span className="text-destructive font-medium">{alert.triggerCount}</span>/{alert.threshold}次
            </span>
          ) : (
            <span>
              {alert.windowMin}分钟内已限制 <span className="text-destructive font-medium">{alert.triggerCount}</span>/{alert.threshold}次
            </span>
          )}
        </div>
...
export function CpuLimitAlertNotification({ enabled }: CpuLimitAlertNotificationProps) {
  const { alerts, unreadCount, markAllRead } = useCpuLimitAlerts(enabled);
  ...
  return (
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={unreadCount > 0 ? `${unreadCount} 条 CPU 限制告警` : 'CPU 限制告警'}
          title={unreadCount > 0 ? `${unreadCount} 条 CPU 限制告警` : 'CPU 限制告警'}
        >
      ...
      >
        <div className="px-3 py-2 border-b border-border flex items-center justify-between">
          <span className="text-sm font-medium text-foreground">CPU 限制告警</span>
          {unreadCount > 0 && (
            <button
              type="button"
    ...
    <CpuLimitAlertHistoryDialog open={historyOpen} onOpenChange={setHistoryOpen} />
    </>
  );
}
