...
import { Loader2, ChevronLeft, ChevronRight, AlertTriangle, Cpu, Server as ServerIcon, AlertOctagon } from 'lucide-react';
import type { CpuLimitAlertLog } from '@/lib/services/cpu-limit-manager';

interface CpuLimitAlertHistoryDialogProps {
  open: boolean;
...
export function CpuLimitAlertHistoryDialog({ open, onOpenChange }: CpuLimitAlertHistoryDialogProps) {
  const [items, setItems] = useState<CpuLimitAlertLog[]>([]);
  ...
  const fetchLogs = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/cpu-limit?action=listAlerts&page=${page}&perPage=${perPage}`);
      if (res.ok) {
          ...
          {loading ? (
            ...
            <div className="space-y-1.5">
              {items.map(alert => {
                ...
                const badgeClass = isReleaseFailure || isNode
                  ? 'border-destructive/40 text-destructive'
                  : 'border-amber-500/40 text-amber-600 dark:text-amber-400';
                const levelLabel = isReleaseFailure ? '解除失败' : isNode ? '节点级' : '实例级';
                const target = isNode
                  ? `节点 ${alert.nodeName}`
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <Badge variant="outline" className={`text-[9px] px-1 py-0 h-3.5 ${badgeClass}`}>
                            {levelLabel}
                          </Badge>
                        ...
                        <div className="mt-1 text-[10px] text-muted-foreground flex items-center gap-1.5 flex-wrap">
                          {isReleaseFailure ? (
                            ...
                            <span>
                              {alert.windowMin}分钟内限制 <span className="text-destructive font-medium">{alert.triggerCount}</span>/{alert.threshold}次
                            </span>
                          )}
                        </div>
