...
import type { BandwidthAlertConfig } from '@/lib/services/bandwidth-manager';

/**
 * 带宽告警配置卡片
 * 管理员在 BandwidthSheet 中配置：时间窗口 + 实例阈值 + 节点阈值
 * 达阈值时右上角告警铃铛显示通知
 */
export function BandwidthAlertConfigCard() {
  const [config, setConfig] = useState<BandwidthAlertConfig>({
    enabled: false,
    windowMin: 60,
    instanceThreshold: 3,
    nodeThreshold: 5,
  ...
  const fetchConfig = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/bandwidth?action=getAlertConfig');
      const data = await res.json();
      if (data.success && data.data) {
        const c = data.data as BandwidthAlertConfig;
        setConfig(c);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const newConfig: BandwidthAlertConfig = {
    ...
    <div className="bg-card rounded-lg p-3 border border-border space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-foreground">限速告警通知</span>
        </div>
        <Switch checked={config.enabled} onCheckedChange={handleToggleEnabled} disabled={loading} />
      </div>
      <p className="text-[10px] text-muted-foreground -mt-1">
        统计窗口内实例/节点触发限速次数达阈值时，右上角铃铛通知管理员
      </p>
